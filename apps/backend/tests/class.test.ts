...
// Each test manages its own data (Bun runs beforeAll across sibling describes first)
...
describe("list", () => {
  ...
  it("returns all classes with relations", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-2025/2026" } });
  ...
  it("orders by year descending then name ascending", async () => {
    await cleanDb();
    const year1 = await prisma.academicYear.create({ data: { year: "CL-ORD-2024/2025" } });
    const year2 = await prisma.academicYear.create({ data: { year: "CL-ORD-2025/2026" } });
...
describe("getById", () => {
  it("returns class by id with all relations", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-GBI-2025/2026" } });
    ...
    expect(found.academicYear.year).toBe("CL-GBI-2025/2026");
  });
  ...
  it("returns null homeroomTeacher when not assigned", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-GBI2-2026/2027" } });
...
describe("create", () => {
  it("creates a class", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-CR-2025/2026" } });
  ...
  it("rejects duplicate class name in same year", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-CR2-2027/2028" } });
  ...
  it("allows same class name in different year", async () => {
    await cleanDb();
    const y1 = await prisma.academicYear.create({ data: { year: "CL-CR3-2030/2031" } });
    const y2 = await prisma.academicYear.create({ data: { year: "CL-CR3-2031/2032" } });
...
describe("getStudents", () => {
  it("returns students for a class", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-GS-2025/2026" } });
    const cls = await prisma.class.create({ data: { name: "Kelas 5A", academicYearId: year.id } });
    await prisma.student.create({ data: { name: "S1", classId: cls.id, nis: "GS-1001", nisn: "GS-001001", gender: "L" } });
    await prisma.student.create({ data: { name: "S2", classId: cls.id, nis: "GS-1002", nisn: "GS-001002", gender: "P" } });
    expect((await service.getStudents(cls.id)).length).toBe(2);
  });

  it("returns students sorted by name", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-GS2-2025/2026" } });
    const cls = await prisma.class.create({ data: { name: "Kelas 5B", academicYearId: year.id } });
    await prisma.student.create({ data: { name: "Z", classId: cls.id, nis: "GS-2001", nisn: "GS-002001", gender: "L" } });
    await prisma.student.create({ data: { name: "A", classId: cls.id, nis: "GS-2002", nisn: "GS-002002", gender: "P" } });
  ...
  it("returns empty array for class with no students", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-GS3-2026/2027" } });
    const cls = await prisma.class.create({ data: { name: "Empty Class", academicYearId: year.id } });
    expect(await service.getStudents(cls.id)).toEqual([]);
  });
});
...
describe("assignTeacher", () => {
  it("assigns homeroom teacher and creates audit log", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-AT-2025/2026" } });
  ...
  it("replaces existing teacher and logs previous", async () => {
    await cleanDb();
    const year = await prisma.academicYear.create({ data: { year: "CL-AT2-2025/2026" } });
